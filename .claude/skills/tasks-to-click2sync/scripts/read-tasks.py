...
def main():
    config = load_config()
    # Current week only. There is intentionally NO override (e.g. env var):
    # the week pulled here must always match the dates written by
    # row-builder.py and the week tab in json-writer.py, which both use the
    # current week. Allowing another week would mislabel and overwrite data.
    ...
    json_start = next(
        (i for i, line in enumerate(stdout_lines) if line.strip().startswith("[")), None
    )
